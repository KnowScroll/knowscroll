 ...
 * honestly. `settleInquiries` is the recovery sweep: expired leases, leaseless Jobs, stale or expired
 * queued Jobs and terminal Jobs whose inquiry is still open. Nothing here ever re-sends a request.
 */
...
/** What the worker may send for an admitted inquiry attempt: the bytes rebuilt from the sealed
...
export async function loadInquiryWork(db: pg.Pool | pg.PoolClient, jobId: string, attemptId: string): Promise<InquiryWork> {
  ...
  return {
    universeId: inquiry.universe_id, privacyEpoch: inquiry.privacy_epoch, jobId, stepId: inquiry.step_id!, contextId: inquiry.context_id, inquiryId: inquiry.id,
    requestId: inquiry.request_id!, requestHash: hash, inputTokensUpperBound: inquiry.input_bytes, maxOutputTokens: route.max_output_tokens,
 ...
 * failure stores only its reasons; "none" stores nothing; stale context discards the reply.
 */
export async function applyInquiryReply(client: pg.PoolClient, f: InquiryFence, text: string, authority: ReasoningAuthority): Promise<InquiryOutcome> {
  ...
  const reply = parseBridgeInquiryReply(text, payload.pairs);
  if (reply.kind === 'shape') {
    await closeInquiry(client, inquiry.id, 'rejected', { attemptId: f.attemptId, reasons: reply.reasons });
    await finish(client, f, false);
  ...
  try {
    decided = await submitBridgeProposal(client, {
      scope: { kind: 'universe', universeId: f.universeId, privacyEpoch: f.privacyEpoch }, proposerKind: 'model', proposerRef: f.attemptId, payload: reply.payload,
    });
...
async function withdrawIdleInquiry(client: pg.PoolClient, row: { job_id: string; universe_id: string; privacy_epoch: number }, decide: (inquiry: InquiryRow) => Promise<{ status: string; reasons: string[]; expire: boolean } | null>): Promise<boolean> {
  ...
  const attempt = (await client.query<{ id: string }>('SELECT id FROM reasoning_attempt WHERE job_id=$1 ORDER BY id DESC LIMIT 1', [row.job_id])).rows[0];
...
export async function settleInquiries(pool: pg.Pool, input: { owner: string; limit?: number }): Promise<number> {
  ...
  let settled = 0;
  const idle = (await pool.query<{ job_id: string; universe_id: string; privacy_epoch: number; attempts: number }>(
    `SELECT j.id AS job_id, j.universe_id, j.privacy_epoch, (SELECT count(*)::int FROM reasoning_attempt at WHERE at.job_id = j.id) AS attempts
  ...
  for (const row of idle) {
    try {
      const closed = await inTransaction(pool, client => withdrawIdleInquiry(client, row, async inquiry => {
        ...
        if (row.attempts > 0) {
          const state = (await client.query<{ state: string }>(`SELECT ac.state FROM reasoning_attempt at JOIN reasoning_accounting ac ON ac.attempt_id = at.id
            WHERE at.job_id=$1 ORDER BY at.id DESC LIMIT 1`, [row.job_id])).rows[0]?.state;
        ...
        const check = await validateInquiryContext(client, { universeId: row.universe_id, privacyEpoch: row.privacy_epoch, jobId: row.job_id,
          stepId: inquiry.step_id!, contextId: inquiry.context_id!, policyVersion: inquiry.policy_version! }, resolveInquiryPolicy, 'lock');
