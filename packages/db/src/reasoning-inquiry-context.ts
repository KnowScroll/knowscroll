...
export async function readInquiryInputs(client: pg.PoolClient, universeId: string, privacyEpoch: number): Promise<InquiryCandidateInput> {
  ...
  // A pair counts as asked in this epoch once a request for it was sent (the database records that when
  // its inquiry closes, #153; one open at a time is never reopened), until a bridge between it is revoked
  // after that inquiry closed: then it may be looked for again from current evidence (ADR-0042 §5.3).
  const asked = (await client.query<{ a: string; b: string }>(
    `SELECT p->'a'->>'code' AS a, p->'b'->>'code' AS b FROM background_inquiry i CROSS JOIN jsonb_array_elements(i.pairs) p
     WHERE i.universe_id = $1 AND i.privacy_epoch = $2 AND i.sent
