...
/** A binding the Composer serves: still standing, still eligible (rechecked at serve time) and never
 * yet shown to this reader. */
const SERVABLE = (binding: string, universe: string) => `${binding}.status = 'active' AND scroll_is_eligible(${binding}.asset_id)
  AND NOT EXISTS (SELECT 1 FROM exposure e WHERE e.universe_id = ${universe} AND e.asset_id = ${binding}.asset_id)`;
...
const DEMANDS = `SELECT d.id, d.status, d.decision, d.reason, c.code, c.name, d.causes, d.created_at, d.changed_at,
    b.asset_id, a.title, b.status AS binding_status, b.origin_bridge_id, b.origin_exposure_id, coalesce(${SERVABLE('b', 'd.universe_id')}, false) AS servable
...
/** The live demand for each concept that has one, for the places anchored on them. A bound Scroll is
 * named only while it would be served: once read, or no longer eligible, it is not new any more. */
export async function placeDemands(client: pg.PoolClient, universeId: string): Promise<Map<string, PlaceDemand>> {
  const rows = (await client.query<Row>(`${DEMANDS} WHERE d.universe_id = $1 AND d.status IN ('waiting','bound','cannot_meet')`, [universeId])).rows
    .filter(r => r.status !== 'bound' || r.servable);
...
/** Bindings the Composer serves first ([SERVABLE]). */
export async function loadBoundScrolls(client: pg.PoolClient, universeId: string): Promise<V3Bound[]> {
  return (await client.query<{ id: string; demand_id: string; asset_id: string; code: string; place_id: string | null; origin_bridge_id: string | null; origin_exposure_id: string | null }>(
    `SELECT b.id, b.demand_id, b.asset_id, c.code, b.place_id, b.origin_bridge_id, b.origin_exposure_id
     ...
     WHERE b.universe_id = $1 AND ${SERVABLE('b', '$1')}
