...
import {
  applyInquiryReply, failInquiry, giveBackUnsentInquiry, loadInquiryWork, settleInquiries, type InquiryOutcome, type InquiryWork,
...
import { createReadinessGate, executeAnswerClaim, type AnswerObservation, type AnswerPass, type AnswerTransport } from './answer-worker.ts';
import { invokeReasoningOnce, type SingleInvocationTransport } from './invoke.ts';

/** The same transport seam as answers: exactly the reserved bytes, the minimal receipt, the text. */
...
type Gates = Partial<Record<'fixture' | 'minimax', ReturnType<typeof createReadinessGate>>>;
...
export async function runInquiryPass(deps: {
  ...
  for (const check of [gate(transport, deps.readiness), ...(answerTransport ? [gate(answerTransport, deps.answers?.readiness)] : [])]) {
  ...
  if (await jobFamily(pool, scheduled.claim.jobId) === 'answer') {
    return { kind: 'answer', pass: await executeAnswerClaim({ pool, owner, transport: answerTransport!, signal, authority }, scheduled), opened };
  }
  return { ...await executeInquiryClaim({ pool, owner, transport, signal, authority }, scheduled), opened };
}
...
export async function executeInquiryClaim(deps: { pool: pg.Pool; owner: string; transport: InquiryTransport; signal: AbortSignal; authority: ReasoningAuthority },
  ...
  const inquiry = (await pool.query<{ id: string; step_id: string }>('SELECT id, step_id FROM background_inquiry WHERE job_id=$1', [claim.jobId])).rows[0]!;
  ...
  // The minimal receipt never carries content (ADR-0012); the text waits here for the parser.
  ...
  try {
    outcome = await inTransaction(pool, async client => {
      if (invocation.kind === 'recorded' && observed === 'success' && text !== null) return applyInquiryReply(client, fence, text, authority);
