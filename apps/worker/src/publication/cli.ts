/**
 * ADR-0024/0025 — the publication-gate "worker": a one-shot process (no queue table exists for this
 * slice; a decision is requested for one named generated Reel, not polled for). Run as
 * `pnpm exec tsx apps/worker/src/publication/cli.ts evaluate --reel-id <uuid> --policy <version>
 * [--decide test_eligible] [--media-root <absolute path>]`, `... mint --reel-id <uuid>` or
 * `... withdraw --reel-id <uuid>`. Holds no provider credentials; makes no network call; every DB
 * row it touches is named on its own command line, never discovered. Prints one JSON line and
