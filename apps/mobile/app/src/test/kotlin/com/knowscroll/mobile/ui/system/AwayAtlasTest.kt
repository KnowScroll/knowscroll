...
class AwayAtlasTest {
    ...
    private val found = InquiryFound(
        ...
        evidence = listOf(InquiryEvidence("clm.gravity.sun_holds_earth", "The Sun's gravity holds Earth in its orbit.", "mechanism", "NASA · Our Sun: Facts", "https://science.nasa.gov/sun/facts/")),
    )
    private val foundItem = AwayItem.ConnectionFound("2026-09-24T10:05:00.000Z", "11111111-1111-4111-8111-111111111111", found)
    private val actions = mutableListOf<String>()

    private fun render(places: List<AtlasPlace> = listOf(planet), items: List<AwayItem> = listOf(foundItem), connections: Map<String, ConnectionState> = emptyMap()) {
        val context = ApplicationProvider.getApplicationContext<android.content.Context>()
        Settings.Global.putFloat(context.contentResolver, Settings.Global.ANIMATOR_DURATION_SCALE, 0f)
        val away = AwayControls(
            state = AwayState.Loaded(AwayResponse(4, null, items, 0, false)), acknowledge = ReturnActionState.Idle, connections = connections,
            onMarkSeen = { actions += "seen" }, onRetryMarkSeen = {},
            connection = ConnectionActions(onKeep = { actions += "keep:$it" }, onRetryKeep = {}, onSeemsWrong = { actions += "wrong:$it" }, onRetrySeemsWrong = {}),
        )
        composeRule.setContent {
            KnowScrollTheme {
                SystemScreen(
                    state = loadedWorlds, atlasState = AtlasState.Loaded(AtlasResponse("cartographer-v1", places, emptyList(), emptyList())),
                    onReturn = {}, onRetry = {}, onEnterScroll = {}, onOpenKeep = {}, away = away,
                )
            }
        }
    }
    ...
    fun aFoundConnectionOpensItsEvidenceOverTheMapAndClosesBackToIt() {
        ...
        assertEquals(listOf("keep:$bridgeId", "wrong:$bridgeId"), actions)
    ...
    fun aKeptConnectionSaysSoInItsSheet() {
        render(connections = mapOf(bridgeId to ConnectionState(kept = true)))
        composeRule.onNodeWithText(line).performClick()
