 ...
 * epoch the reader is still in, and nothing purged private state meanwhile, the answer is watched,
 * whatever the reader did: navigation only decides whether the sheet shows it. Returns whether the
 * answer is now watched.
 */
internal suspend fun requestWatchedAnswer(
    store: StateStore, answer: WatchedAnswer, send: suspend (PendingAnswerRequest) -> AnswerRequestReceipt, epochIsCurrent: () -> Boolean,
): Boolean {
    ...
    return epochIsCurrent() && store.watchAnswer(answer, pending.clientRequestId)
}
