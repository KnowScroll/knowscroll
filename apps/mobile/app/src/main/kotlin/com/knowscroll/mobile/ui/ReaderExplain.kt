 ...
 * actually returned (the feed/Trace `reason`, `truthState`, the encounter's `kind` and the
 * client's own discovery/saved-Trace origin). No interest, learning, profile or evidence path is
 * invented (Law 4, Law 11, Law 13). Kept as pure functions so state transitions and
 * copy stay unit-testable without an Android runtime. The Scroll and Reel readers share them
 * (#167), so each sentence names the kind of encounter on screen. */
...
internal fun noReasonRecorded(kind: String): String = "No explanation was recorded for this $kind."
...
internal fun explainOriginText(origin: ReaderOrigin, kind: String): String = when (origin) {
    ReaderOrigin.Discovery -> "You opened this $kind through deliberate discovery from your universe."
