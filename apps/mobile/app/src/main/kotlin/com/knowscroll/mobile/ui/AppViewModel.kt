...
sealed interface ScrollState {
    ...
    data class Reading(val item:ScrollItem,val exposureId:String,val eventId:String,val keep:KeepState):ScrollState
...
class AppViewModel(application:Application,private val savedState:SavedStateHandle):AndroidViewModel(application) {
    ...
    init {
        if(savedState.get<String>("screen")=="scroll") session?.let { show(it) }
        loadUniverse()
    }
    ...
    private fun loadNext(){
        busy=true;_screen.value=Screen.Scroll("");savedState["screen"]="scroll";_scroll.value=ScrollState.Loading
        session?.let{visited.add(it.item.assetId)}
        viewModelScope.launch {
            ...
            } catch(e:Exception){_scroll.value=ScrollState.Unavailable(message(e))} finally{busy=false}
        }
    }
    private fun show(s:ScrollSession){
        _screen.value=Screen.Scroll(s.item.assetId);savedState["screen"]="scroll"
        _scroll.value=ScrollState.Reading(s.item,s.exposureId,s.exposureEventId,if(s.keepJobId.isEmpty())KeepState.Idle else KeepState.Kept(s.keepJobId))
    }
    ...
    fun onVisible(assetId:String){
        ...
        viewModelScope.launch {
            busy=true
            try { val next=recordExposure(s);session=next;show(next) }
    ...
    private suspend fun recordExposure(s:ScrollSession):ScrollSession {
        ...
        return s.copy(exposureId=r.exposureId,exposureEventId=r.eventId).also{store.write(it)}
    }
    fun keep(){
        if(busy)return
        val s=session ?: return
        ...
        viewModelScope.launch {
            try {
                ...
                val kept=exposed.copy(keepJobId=receipt.jobId,keepEventId=receipt.eventId)
            ...
            } catch(e:Exception){_scroll.value=current.copy(keep=KeepState.Failed(message(e)))} finally{busy=false}
        }
    }
    fun returnToUniverse(){
        if(busy){_toast.value="Finishing this request. You can return in a moment.";return}
        _screen.value=Screen.Universe;savedState["screen"]="universe";loadUniverse()
    }
