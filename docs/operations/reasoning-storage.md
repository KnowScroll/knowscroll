...
- `purgeClosedReasoningAccounting` performs bounded cleanup after all duties have been closed for at least 30 days. Financial closure alone is insufficient. Held liability, unconfirmed remote duties, reconciliation or idempotency holds block removal. A surviving private Attempt also blocks cleanup; [Separate scheduled maintenance](reasoning-retirement.md) now retires eligible cancelled/expired private graphs after seven days and runs this accounting purge. Completed/failed private graphs and unstamped legacy Jobs remain outside the retirement policy. Eligible accounting deletion cascades retained evidence; direct child deletion stays forbidden.
