...
Amendment (#177, 2026-09-25): a scheduler's probes run one at a time, under a transaction advisory lock per policy version taken before anything else. Every probe that progresses moves the one generation, so concurrent probes gained nothing and could fence each other: a probe that found the head's universe held by another probe committed a blocked round under it, and two in step did so until both had spent their probe budget (CI admitted neither once). The lock is acquired while nothing else is held, so it adds no wait-for edge to the order above.
