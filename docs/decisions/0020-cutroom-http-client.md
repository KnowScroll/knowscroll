...
Each operation makes one fetch with a finite timeout and external abort propagation. Requests are capped at256KiB. Responses default to1MiB, configurable from1byte through4MiB, and are bounded while streaming even without Content-Length. Timeout defaults to5seconds, configurable1–60,000ms. Identities are non-empty well-formed Unicode capped at1,024UTF-8 bytes; runIds cannot be dot path segments. Copy run-reference bindings at call entry so caller mutation during a fetch cannot change response authority. A stream error, timeout, abort or connection failure returns a fixed transport outcome with write uncertainty for submit/cancel; it never exposes raw errors/response bodies in logs. Malformed or mismatched protocol responses are distinct from transport failures and declared engine refusals. A pre-aborted operation performs no fetch. No request, output path or provider detail is logged by this client.
