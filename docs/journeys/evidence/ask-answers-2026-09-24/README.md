...
Every receipt below except the two labelled v1 runs was produced from commit `5ad8db7` (clean tree),
after the fresh-context review fixes of PR #143.

## What was proven

| Proof | How | Result |
| --- | --- | --- |
| Core rules | `tests/ask-answer-core.test.ts` (8) | exact request bytes; verbatim quotes; invented, all-space or near-empty quotes refused; empty limits and NUL refused; reader characterisation (incl. contractions and curly apostrophes) refused, ordinary second person allowed; bad shapes with sub-codes |
...
| Nothing stuck after admission | `tests/ask-answers-recovery.test.ts` (2), one remote slot | a worker stopping before dispatch gives the attempt back (all reservations released, answer `failed`/`not_sent`); a worker dying after admission is closed by the recovery sweep once its lease expires (`worker_stopped`); the next Ask then proceeds |
...
| Android | 125 unit tests + lint | Ask sheet states; after an unclear failure "Get an answer" is offered again with the saved key; the question is sent literally; disclosure copy |
...
| `live-sample-v2.json` | `5ad8db7` | same, ×5 | 5 × answered, 3 verified quotes each |
| `android-live-*` | `5ad8db7` | same, on the emulator | answered, 3 quotes |

Total provider requests: 22 of the 40 authorized (session ledger under `$KS_DEV_ROOT`).
